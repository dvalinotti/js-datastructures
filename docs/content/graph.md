...
A Graph is a data structure that consists of nodes (values) and edges (connections between values). There are a few ways that you can represent a Graph, such as with Node **objects that have pointers** to other nodes, or with an **adjacency list** that holds key value pairs of nodes and their neighbors.

![](/graph-vertices-edges_0.webp)
